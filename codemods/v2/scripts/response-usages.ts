...
async function transform(root: SgRoot<TSX>): Promise<string> {
	...
	arrowFunctions.forEach((arr) => {
		if (
			!isMSWCall(arr.getMatch("MEMBER")?.text() ?? "", "msw", "http", rootNode) &&
			!isMSWCall(arr.getMatch("MEMBER")?.text() ?? "", "msw", "graphql", rootNode)
		)
			return;
		const prms = arr.getMatch("PARAMS");
		const paramsText = prms?.text() ?? "";
		const params = paramsText.substring(1, paramsText.length - 1).split(",") as string[];
		...
		const block = arr.getMatch("BLOCK");
		if (resParam) {
			const response = block?.findAll({
				rule: {
					kind: "call_expression",
			...
			response?.forEach((res) => {
				const content = res.getMultipleMatches("CONTENT");
				let delay = "";
				...
				content.forEach((c) => {
					...
					isHeaders.forEach((head) => {
						const name = head.getMatch("NAME")?.text() ?? "";
						const content = head.getMatch("CONTENT")?.text() ?? "";
						if (name.substring(1, name.length - 1).toLowerCase() === "content-type") {
							let responseType = {
								"text/plain": "text",
								"text/html": "html",
								"application/xml": "xml",
							...
							responseType = typeof responseType === "undefined" ? "text" : (responseType ?? "");
						}
						headers[`${name.substring(1, name.length - 1)}`] = content;
					});
					// cookies
					const isCokies = c.findAll({
						rule: {
							kind: "call_expression",
					...
					if (isText) {
						const c = isText.getMatch("CONTENT")?.text() ?? "";
						responseContent = c;
						responseType = "text";
					}
					const isBody = c.find({
					...
					if (isBody) {
						const c = isBody.getMatch("CONTENT")?.text() ?? "";
						responseContent = c;
					}
					// if it's json
					const isJson = c.find({
					...
					if (isJson) {
						const c = isJson.getMatch("CONTENT")?.text() ?? "";
						responseContent = c;
						responseType = "json";
					}
					// if it's json
					...
					const isErrors = c.find({
						rule: {
							kind: "call_expression",
							pattern: `${ctxParam}.errors($CONTENT)`,
						},
					});
					const isExtensions = c.find({
						rule: {
							kind: "call_expression",
				...
				if (delay) {
					if (hasMswImport) {
						...
						if (importText[0] === "{" && importText[importText.length - 1] === "}") {
							importText = `${importText.substring(0, importText.length - 1)}, delay}`;
						} else {
							importText += "{delay}";
						}
						edits.push(hasMswImport.replace(`import ${importText} from "msw"`));
					} else {
						const newRoot = `import { delay } from "msw"; \n ${rootNode.text()}`;
						edits.push(rootNode.replace(newRoot));
					}
					newBlock = `async ${paramsText} {\n  await delay(500); ${block?.text().substring(1)}`;
				}
				if (cookies.length) {
					let textCookies = "";
				...
				const secParam = `{${status ? `status: ${status},\n` : ""}${
					Object.keys(headers).length ? `headers: {${headersText}},\n` : ""
				}}`;
				const thirdParam = `${data ? `data: ${data},\n` : ""}${
				...
				newBlock = newBlock.replace(
					...
					}${thirdParam !== "{}" ? thirdParam : ""}${secParam !== "{}" ? secParam : ""})`,
				);
			});
	...
	if (arrowFunctions.length && edits.length) {
		const haveHttpResponse = rootNode.findAll({
			rule: {
				kind: "import_statement",
				has: {
					kind: "import_clause",
		...
		if (!haveHttpResponse.length) {
			if (hasMswImport) {
				...
				let importText = importMatch?.text() ?? "";
				if (importText[0] === "{" && importText[importText.length - 1] === "}") {
					importText = `${importText.substring(0, importText.length - 1)}, HttpResponse}`;
				} else {
